...
template <typename _ForeignKeyType_ = String_t> class FSController
{
  ...
  public:
	 ...
	 *
	 */
	__0x_attr_FSC_gss constexpr std::size_t &GetStackRegisterSize(void) noexcept
	{
		std::lock_guard<std::mutex> _lock(this->_mtx_guard);
	 ...
	 *
	 */
	__attribute__((no_icf, always_inline, access(read_only, 1), access(read_only, 2), optimize(ATTR_OPTIMIZE_LEVEL))) inline const bool CreateFile(const StringView_t &_file_name, const StringView_t &_content = "")
	{
	...
	/**
	 * 
	 * Collect directory entries from _directory, if _recursive is true, it will traverse the entire directories within _directory.
	 ...
	 * 
	*/
	__attribute__((no_icf, warn_unused_result, access(read_only, 1), optimize(ATTR_OPTIMIZE_LEVEL))) const std::vector<String_t> CollectDirectoryEntries(const StringView_t &_directory, const bool _recursive = false)
	{
	...
	/**
	 *
	 * Collect directory entries if any, constructs an object containing min/max collection entry size, along with
	 * an std::array<string> register containing entry name(path) and a collection array size.
	 * @param StringView_t& directory to collect and profile
	 * @param bool bit flag mandating recursive iteration or not
	 * @param std::size_t threshold to use for register allocation, if register size do not exceed or touch the threshold, structure will be deleted and empty block returned.
	 * @returns stDirectoryCollectionStat a structure containing directory profiling information, register stack, register size, min/max register entry size.
	 *
	 */
	...
	/**
	 *
	 * Scan directory recursively, aggregate every entry which is a regular file type and return the aggregation.
	 ...
	 *
	 */
	__attribute__((no_icf, cold, warn_unused_result, stack_protect, zero_call_used_regs("used"), access(read_only, 1), access(read_only, 2), optimize(ATTR_OPTIMIZE_LEVEL))) const bool CreateDirectoryBackup(const StringView_t &dir_source, const StringView_t &dir_dest,
																																																			  const bool dest_override = false, const bool copy_empty_files = false)
	{
		try
		{
			...
			if (!is_source)
				throw std::filesystem::filesystem_error::runtime_error("source directory not valid for backup!");
			if (!is_destination)
				std::filesystem::create_directories(dir_dest);

			if (IsDirectory(dir_dest))
			{
				std::cout << "destination backup path created!\n";
	...
	/**
	 * 
	 * Check if directory contains a specific file type of file name.
	 ...
	 * 
	*/
	__attribute__((cold, warn_unused_result, access(read_only, 1), access(read_only, 2), optimize(ATTR_OPTIMIZE_LEVEL))) inline const stDirectoryLookup DirectoryContainsFileType(const StringView_t &directory, const StringView_t &file_type)
	{
		if (directory.empty() || file_type.empty() || directory.size() > FS_MAX_FILE_NAME_LENGTH || file_type.size() > FS_MAX_FILE_NAME_LENGTH)
			return {};
		std::function<String_t(const StringView_t &path)> split_path = [&](const StringView_t &path) { return path.find("/") != std::string::npos ? static_cast<String_t>(path.substr(path.find_last_of("/") + 1).data()) : String_t(""); };
		stDirectoryLookup lookup_table;
		lookup_table.has_found = false;
		lookup_table.file = "";
		lookup_table.path = "";
		std::vector<String_t> dir_entries = this->CollectDirectoryEntries(directory, true);
		if (dir_entries.size() > 0)
		{
			for (std::size_t j = 0; j < dir_entries.size(); ++j)
			{
				if (split_path(static_cast<StringView_t>(dir_entries[j])).find(file_type) != std::string::npos)
				{
					lookup_table.file = split_path(dir_entries[j]);
					if (file_type.find(".") != std::string::npos && lookup_table.file.find(".") == std::string::npos)
						lookup_table.file.clear();

					lookup_table.path = lookup_table.file.empty() ? "" : dir_entries[j];
					lookup_table.has_found = lookup_table.file.empty() ? false : true;
				}
			}
		}
		return lookup_table;
  ...
  private:
	...
	/**
	 *
	 * Handle file streaming operations, handles _create_new value, if true, this will create an empty or not-empty file based
	 * on _buffer value.
