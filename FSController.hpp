...
/*                      Enumeration                      *\
\*+++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
enum class eFileDescriptorMode : uint8_t { READ = 0, WRITE };
...
private:
	 ...
	 * @param bool read_mode Whether to open the map in read-only mode (default) or read-write mode.
	 * @param size_t offset_size The offset to the starting position of the map (default is 0).
	 *
	 ...
	 * @param eFileDescriptorMode _mode The mode in which to open the file (read-only or read-write).
	 *
	 ...
	 * @param fMap_t *__restrict__ mapped_data_pointer The pointer to the mapped memory region.
